...
describe('NFTDaoPool test', function () {
  describe('deposit', async function () {
    ...
    it('should deposit NFT tokens to pool', async function () {
      ...
      await token.approve(daoPool.address, 1);
      expect(await token.balanceOf(account.address)).to.eq(1);
      expect(await token.balanceOf(daoPool.address)).to.eq(0);
      ...
      expect(await token.balanceOf(daoPool.address)).to.eq(1);
    ...
    it('should deposit multiple times NFT tokens to pool', async function () {
      ...
      await token.approve(daoPool.address, 1);
      await token.approve(daoPool.address, 2);
      ...
      //then
      expect(await token.balanceOf(account.address)).to.eq(0);
      expect(await token.balanceOf(daoPool.address)).to.eq(2);
  ...
  describe('withdraw', async function () {
    ...
    it('should withdraw some of user (msg.sender) tokens in pool', async function () {
      ...
      expect(accountTokensBalanceAfter.sub(accountTokensBalanceBefore)).to.eq(1);
    });
    ...
    it('should withdraw 2 of user (msg.sender) when withdraw 2 times', async function () {
      ...
      expect(await daoPool.balanceOf(account.address)).to.eq(0);
      const accountTokensBalanceAfter = await token.balanceOf(account.address);
      expect(accountTokensBalanceAfter.sub(accountTokensBalanceBefore)).to.eq(2);
    });
  });
  ...
  describe('resolveProposal', async function () {
    ...
    it('should revert when proposal not ended yet', async function () {
      ...
      //when and then
      await expect(NFTDaoPool.resolveProposal(proposal.address)).to.revertedWith('Proposal not ended');
    });
    ...
    it('should resolve proposal without token votes', async function () {
      ...
      expect(await NFTDaoPool.approvedProposals(proposal.address)).to.eq(true);

      //when
      await NFTDaoPool.resolveProposal(proposal.address);

      //then
      expect((await NFTDaoPool.getProposalForVoters(proposal.address)).length).to.eq(0);
      expect((await NFTDaoPool.getProposalAgainstVoters(proposal.address)).length).to.eq(0);
      expect(await NFTDaoPool.approvedProposals(proposal.address)).to.eq(false);
      expect(await NFTDaoPool.voterActiveProposals(account.address)).to.eq(0);
      expect(await NFTDaoPool.balanceOf(proposal.address)).to.eq(0);
    });
    ...
    it('should resolve proposal with token vote for won side', async function () {
      ...
      await mintNft(token as ERC721Development, account.address);
      await token.approve(NFTDaoPool.address, 1);
      await NFTDaoPool.deposit(1);
      await proposal.voteWithToken(true);
      await waitForProposalToEnd(proposal);

      expect((await NFTDaoPool.getProposalForVoters(proposal.address)).length).to.eq(1);
      expect((await NFTDaoPool.getProposalAgainstVoters(proposal.address)).length).to.eq(0);
      expect(await NFTDaoPool.approvedProposals(proposal.address)).to.eq(true);
    ...
    it('should resolve proposal with token vote for lost side (voter loose everything)', async function () {
      ...
      await tokenByOtherAccount.approve(NFTDaoPool.address, 2);
      ...
      await waitForProposalToEnd(proposal);
      expect((await NFTDaoPool.getProposalForVoters(proposal.address)).length).to.eq(1);
      expect((await NFTDaoPool.getProposalAgainstVoters(proposal.address)).length).to.eq(1);
      expect(await NFTDaoPool.approvedProposals(proposal.address)).to.eq(true);
      expect(await NFTDaoPool.voterActiveProposals(account.address)).to.eq(1);
      ...
      expect(tokensBalanceOfDaoAfterResolveProposal.sub(tokensBalanceOfDaoBeforeResolveProposal)).to.eq(1);
    });
  });
});
