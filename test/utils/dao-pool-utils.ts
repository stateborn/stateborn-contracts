...
export const depositTokensToPool = async (pool: ERC20DaoPool, amount: number) => {
  ...
  await pool.deposit(ethers.parseUnits(amount.toFixed(0), ERC_20_DECIMALS));
};

export const createErc20DaoPool = async (token: IERC20): Promise<ERC20DaoPool> => {
  return (await ethers.deployContract('ERC20DaoPool', [token.getAddress()])) as ERC20DaoPool;
};

export const createNftDaoPool = async (token: ERC721): Promise<NFTDaoPool> => {
  return (await ethers.deployContract('NFTDaoPool', [token.getAddress()])) as NFTDaoPool;
};
