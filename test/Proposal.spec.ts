...
import {
    ...
    initializeErc20TokenAndDaoAndOtherAccountWith500DaoTokens,
    sleep,
...
describe('Proposal test', function () {
    describe('creation', function () {
        it('should initialize correct ERC-20 proposal', async function () {
            ...
            const sendDaoTokensTx = encodeSendErc20TokensTx(dao, proposalId, tokenAddress, otherAccount.address, tokenTransferAmount);
            ...
            const before = nowInSeconds - 10;
            const after = nowInSeconds + 10;
            ...
            expect(await proposal.daoAddress(), 'Expecting daoAddress correct').to.eq(daoAddress);
            expect(await proposal.daoPool(), 'Expecting daoPool correct').to.eq(erc20DaoPoolAddress);
        ...
        it('should create proposal with 5 ETH collateral', async function () {
            //given
            const {dao, token, account, otherAccount} = await loadFixture(initializeErc20TokenAndDaoFixture);
            const tokenAddress = await token.getAddress();
        ...
        it('should revert when creating proposal with 0 ETH collateral', async function () {
            const {dao} = await loadFixture(initializeErc20TokenAndDaoFixture);
        ...
        it('should revert when creating proposal with too small collateral (0.1 ETH)', async function () {
            const {dao} = await loadFixture(initializeErc20TokenAndDaoFixture);
            await expect(
                dao.createProposal(generateRandomProposalId(), generateRandomMerkleRoot(), [], {
                    value: ethers.parseEther('0.1'),
                })
            ).to.be.revertedWith('Collateral too small');
        });
        ...
        it('should revert when creating proposal with incorrect collateral (1.2 ETH - not multiplication of required collateral)', async function () {
            ...
            ).to.be.revertedWith('Collateral incorrect');
        });
    });
    ...
    describe('vote, voteWithToken', function () {
        it('should increment voter votes when vote for (5 ETH vote)', async function () {
            ...
            const proposalAddress = proposal.getAddress();
            ...
            expectProposalVoteResults(vote, 5, 0, 0, 0);
            expect(await ethers.provider.getBalance(proposalAddress)).to.eq(ethers.parseEther('6'));
        });
        ...
        it('should increment voter votes when vote for 3 times (3x5 ETH vote)', async function () {
            ...
            expect(await ethers.provider.getBalance(proposalAddress)).to.eq(ethers.parseEther('16'));
        });
        ...
        it('should set votes once when vote for with 500 token collateral 3 times (expecting 5 votes)', async function () {
            ...
            expect(await ethers.provider.getBalance(proposalAddress)).to.eq(ethers.parseEther('1'));
        });
        ...
        it('should increment voter votes when vote for and against (10 ETH votes total)', async function () {
            ...
            expect(await ethers.provider.getBalance(proposalAddress)).to.eq(ethers.parseEther('11'));
        });
        ...
        it('should revert when vote with 1 ETH collateral after proposal challenge time ended', async function () {
            ...
            const tokenAddress = await token.getAddress();
            const {dao} = await deployErc20Dao(tokenAddress, 1);
            const proposal = await createSendErc20Proposal(
                dao,
                tokenAddress,
                generateRandomProposalId(),
    ...
    describe('claimReward', function () {
        ...
        it('should reward proposal creator only once (when no voters)', async function () {
            ...
            expect(await ethers.provider.getBalance(proposalAddress)).to.eq(ethers.parseEther('0'));
